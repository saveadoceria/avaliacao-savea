    ...
    <div className="relative min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: '#f3eae1', fontFamily: 'sans-serif' }}>
      <div className="w-full max-w-lg mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 text-gray-800">
          ...
          <button
            onClick={() => {
              const mensagem = `Olá! Recebi uma avaliação da Sávea Doceria:%0A%0A` +
                               `⭐ Atendimento: ${notas.atendimento}/5%0A` +
                               `⭐ Produto: ${notas.produto}/5%0A` +
                               `⭐ Entrega: ${notas.entrega}/5%0A%0A` +
                               `💬 Comentário: ${comentario || 'Nenhum'}`;
              window.open(`https://wa.me/5514988396568?text=${mensagem}`, '_blank');
            }}
            className="w-full mt-8 py-3 rounded-xl font-semibold text-white transition-all hover:opacity-90"
            style={{ backgroundColor: '#444631' }}
          >
            Enviar Avaliação
          </button>
        </div>
